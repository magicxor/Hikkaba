...
public class AttachmentService : IAttachmentService
{
    ...
    private async Task<GenericFileAttachmentSm> ConvertToFileAttachmentSmAsync(
        IFormFile formFile,
        CancellationToken cancellationToken)
    {
        ...
        {
            return new GenericFileAttachmentSm
            {
                BlobId = blobId,
        ...
        {
            return new PictureFileAttachmentSm
            {
                BlobId = blobId,
                AttachmentType = attachmentType,
                FileNameWithoutExtension = fileNameWithoutExtension,
        ...
        return new PictureFileAttachmentSm
        {
            ...
            Width = width,
            Height = height,
            ThumbnailRm = thumbnail,
        };
    }

    [MustDisposeResource]
    public async Task<FileAttachmentCollection> UploadAttachmentsAsync(
        Guid blobContainerId,
        IFormFileCollection formFileCollection,
        CancellationToken cancellationToken)
    {
        var attachments = new FileAttachmentCollection();
        ...
        var thumbnailUploadTasks = attachments
            .OfType<PictureFileAttachmentSm>()
            .Where(attachment => attachment.ThumbnailRm != null)
            .Select(attachment =>
                _storageProvider.SaveBlobStreamAsync(
                    blobContainerId.ToString(),
                    attachment.BlobId + Defaults.ThumbnailPostfix,
                    attachment.ThumbnailRm!.ContentStream,
                    closeStream: false));
