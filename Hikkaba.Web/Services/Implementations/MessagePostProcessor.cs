...
public class MessagePostProcessor : IMessagePostProcessor
{
    ...
    private static readonly Regex UriRegex = new(@"(((https?|ftp):)(//([^\s/?#]*))?([^\s?#]*)(\?([^\s#]*))?(#([^\s]*))?)", RegexOptions.Compiled);
    private static readonly Regex CrossLinkRegex = new(@"&gt;&gt;([a-z0-9\-]+)", RegexOptions.Compiled);
    private static readonly Regex ReplaceLineTerminatorsRegex = new(@"\u000D\u000A|\u000A|\u000B|\u000C|\u000D|\u0085|\u2028|\u2029", RegexOptions.Compiled);
    private static readonly Regex LimitLineTerminatorCountRegex = new(@"(\u000D\u000A){3,}", RegexOptions.Compiled);
